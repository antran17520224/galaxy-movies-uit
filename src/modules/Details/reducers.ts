...
const onHandleCurrentSession = (state: IDetailsState, action: IActions.IHandleCurrentSession) => {
...
const onToggleModal = (state: IDetailsState, action: IActions.IToggleModal) => {
	const { type, codeTrailer, coordinate } = action.payload;
	...
	switch (type) {
		case DETAILS_MODAL.TRAILER_MODAL:
			return {
				...
				codeTrailer: codeTrailer as string,
			};
...
const onGetSessionByMovieId = (state: IDetailsState, action: IActions.IGetSessionByMovieId) => {
	return {
		...state,
		isProcessing: true,
	};
