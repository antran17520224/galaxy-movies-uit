...
export const ShowTimes: React.FC<IProps> = (props) => {
	...
	const [arrayIndexTabPanel, setIndexTabPanel] = React.useState<number[]>([]);
	...
	React.useEffect(() => {
		if (isLoadDate) {
			...
			props.actions.getSessionByMovieId({
				movieId: currentMovie?._id as string,
			});
		}
		if (sessionRecords.length > 0) {
			sessionRecords.map((session) => {
				arrayDayAndMonth.map((day, index) => {
					if (day === moment(session.date).format('L').slice(0, -5).replace('/', '-')) {
									...
									>
										<div className="wrapper-list-showtimes">
											<Grid container className="wrapper-showtime">
												<Grid item lg={3} xs={8} className="theater">
													<div className="text">
														<h2 className="theater-name">
															{session.cinema_id.cinema_Name}
														</h2>
												...
												>
													<>
														<span className="theater-name">
															{session.theaters_id.theaters_Name}
														</span>
												...
												<Grid item lg={6} xs={12} className="wrapper-time">
													<div className="wrapper-button-time">
														<Link
															to={`/ticketing/${session.movie_id.name}/${session.theaters_id.theaters_Name}`}
														>
															<Button
																variant="outlined"
